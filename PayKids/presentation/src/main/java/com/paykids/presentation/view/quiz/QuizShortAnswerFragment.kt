...
@AndroidEntryPoint
class QuizShortAnswerFragment : BaseFragment<FragmentQuizShortAnswerBinding>(), ConfirmDialogInterface {
    ...
    private fun navigateToNextQuiz() {
        ...
        if (quizState is UiState.Success) {
            ...
            when (quiz.quizType) {
                "IMAGE_CHOICE" -> {
                    val action = QuizShortAnswerFragmentDirections
                        .actionQuizShortAnswerFragmentToQuizImageFragment(stageNumber, quizNumber)
                    findNavController().navigate(action)
                }
                "TEXT_CHOICE" -> {
                    val action = if (quiz.imageURL.isNullOrEmpty()) {
                        QuizShortAnswerFragmentDirections
                            .actionQuizShortAnswerFragmentToQuizMultipleChoiceFragment(stageNumber, quizNumber)
                    } else {
                        QuizShortAnswerFragmentDirections
                            .actionQuizShortAnswerFragmentToQuizMultipleChoiceImgFragment(stageNumber, quizNumber)
                    }
                    findNavController().navigate(action)
                }
                "SHORT_ANSWER" -> {
                    val action = if (quiz.imageURL.isNullOrEmpty()) {
                        QuizShortAnswerFragmentDirections
                            .actionQuizShortAnswerFragmentToQuizShortAnswerFragment(stageNumber, quizNumber)
                    } else {
                        QuizShortAnswerFragmentDirections
                            .actionQuizShortAnswerFragmentToQuizShortAnswerImgFragment(stageNumber, quizNumber)
                    }
                    findNavController().navigate(action)
                }
            }
